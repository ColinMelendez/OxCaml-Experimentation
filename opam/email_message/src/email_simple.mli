...
module For_testing : sig
  (** Within the scope of [~f], [make_id] returns deterministic, unique ids of the form
      ["<auto-generated-id-N@test>"] where N increments from 0 on each call. The counter
      resets each time [with_unique_message_ids] is called. The override is cleaned up via
      [Monitor.protect] when [~f] resolves or raises.

      This is useful for tests that need to distinguish between separate [make_id] calls
      (e.g. verifying that multiple envelopes share the same Message-Id). Most tests
      should not need this — the default test behaviour returns a single hard-coded id. *)
  val with_unique_message_ids : f:(unit -> 'a Async.Deferred.t) -> 'a Async.Deferred.t
end
