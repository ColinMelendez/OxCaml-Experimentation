...
let make_id_override : (unit -> string) option ref = ref None

let make_id () =
  match !make_id_override with
  | Some f -> f ()
  | None ->
...
let create
  ...
  Expert.create_raw
    ...
    ?bcc:(Option.map bcc ~f:Email_address.to_string)
...
let create_utf8
  ...
  Expert.create_raw
    ...
    ?bcc:(Option.map bcc ~f:Email_address.to_string_utf8)
  ...
  let with_unique_message_ids ~f =
    ...
    := Some
         (fun () ->
           let n = !counter in
           Int.incr counter;
           sprintf "<auto-generated-id-%d@test>" n);
    Async.Monitor.protect f ~finally:(fun () ->
      make_id_override := None;
      Async.return ())
  ;;
end
