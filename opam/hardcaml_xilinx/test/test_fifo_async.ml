...
module%test Lws_evsim_tests = struct
  open Core
  open Hardcaml_lws
  ...
  let create_lws_evsim ~write_clock_time ~read_clock_time =
    ...
    L.create
      ...
      ~backend_specific_config:
        { common =
            { clocks =
                [ { port_name = "write_clock"
                  ; time = write_clock_time
                  ; initial_delay = 0
                  }
        ...
        ; backend = Lws_evsim.Evsim.Config.default
        }
      (fun scope input ->
        Fifo.hierarchical
          scope
  ...
  let%expect_test "Multi-clock domain write then read, fast writer slow reader" =
    ...
    [%expect
      ...
      │                  ││────────────────────────────────────────────────────────────────│
      └──────────────────┘└────────────────────────────────────────────────────────────────┘
      81fe0c6eb0d7f0d1af9dfd1e27200d19
      |}]
  ;;
  ...
  let%expect_test "Multi-clock domain write then read, slow writer fast reader" =
    ...
    [%expect
      ...
      │                  ││────────────────────────────────────────────────────────────────│
      └──────────────────┘└────────────────────────────────────────────────────────────────┘
      0e377fdf284f49648fe2a289a8c73da7
      |}]
  ;;
