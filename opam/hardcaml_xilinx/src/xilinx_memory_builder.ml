...
module Read_port_2d = struct
  ...
  struct
    module Pre = struct
      ...
      let port_names_and_widths =
        { horizontal_index =
            "horizontal_index", Int.max 1 (Int.ceil_log2 X.horizontal_dimension)
        ; vertical_index = "vertical_index", X.vertical_index_width
  ...
  module Specialize_with_config (The_config : Config.S) = Specialize (struct
      let vertical_index_width = Config.vertical_index_width The_config.t
      let horizontal_dimension = Config.horizontal_dimension The_config.t
    end)
end
