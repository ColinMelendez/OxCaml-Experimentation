...
type section =
  { name : string
  ; addr : int64
  (** The address at which the section's first byte should reside in memory, or 0 for
      non-allocatable sections. *)
  ; offset : int64
  (** The byte offset from the beginning of the ELF file to the first byte in the section *)
  ; size : int64
  }

(** Sectionfields are used in position independent executables to find the dynamic
    addresses of probe sites and semaphores *)
type t =
  { filename : string
  ; inode : int
  ; shared_object : bool
