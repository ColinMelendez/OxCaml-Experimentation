...
module General : sig
  (** Here is a summary of the type parameters in
      [(i -> a -> b, it -> at -> bt, c) Accessor.General.t]:
...
module O : sig
  ...
  (** [a @> b] is the composition of the two accessors [a] and [b]. From left to right, a
      ...
      extract a value from a data structure using a [field], you would probably use [get].
      However, if you compose the [field] with an [optional], [get] no longer makes sense;
      you must use something like [get_option], instead.
  ...
  (** [x.@?(t)] extracts at most one value from [x] as identified by [t]. The non-operator
      name is [Accessor.get_option]. *)
  val ( .@?() )
    :  'at
...
(** An [Index.t] is a heterogeneous stack of values intended to serve as "breadcrumbs"
    ...
    structure. For example, if on the way in you traversed a [Map.t], one component of the
    index might be the key of the data being accessed.
    ...
    need for a [match] expression or a type annotation. This is a lot more convenient than
    the alternative of pattern matching on deeply nested tuples.
...
(** Extract all the values targetted by an accessor in some composite data structure into
    a list. *)
val to_list
  :  (unit -> 'a -> _, unit -> 'at -> _, [> many_getter ]) General.t
  -> 'at
...
(** [for_all t at ~f] returns [true] iff [f] returns [true] for all targets in [at]. This
...
(** [min_elt t at ~compare] uses [compare] to compare each target in [at] and returns the
    first target with the smallest value. *)
val min_elt
...
(** [max_elt t at ~compare] uses [compare] to compare each target in [at] and returns the
    first target with the largest value. *)
val max_elt
  :  (unit -> 'a -> _, unit -> 'at -> _, [> nonempty_getter ]) General.t
  -> 'at
  -> compare:('a -> 'a -> int)
...
(** [map_reduce_nonempty t at ~combine ~f] applies [f] to each targetted value in [at] and
    combines the results using [combine]. [combine] is expected to satisfy the property:
    [combine (combine a b) c = combine a (combine b c)]. *)
...
(** [reduce_nonempty t at ~combine] combines all accessed values in [at] using [combine].
    [combine] is expected to satisfy the property:
    [combine (combine a b) c = combine a (combine b c)]. *)
val reduce_nonempty
  :  (unit -> 'a -> _, unit -> 'at -> _, [> nonempty_getter ]) General.t
  -> 'at
...
(** There are a number of signatures and functors in this section, all for the purpose of
    providing a wide variety of applicative/monadic functionality. *)
...
(** The monad signatures differ from the applicative ones in that some of the functions
    ...
    argument to [`Parallel] then all the side effects are performed up front, and then the
...
(** [Of_applicative] and [Of_applicative2] can be used to generate map-like functions that
    can use applicative effects. See also [Of_monad], which gives more control over the
    relationship between side effects and monadic effects. *)
...
(** [construct] goes the opposite way to most access patterns. It allows you to construct
    a composite data structure without reading from one. *)
...
(** The value restriction will apply frequently to accessors you define. Unfortunately,
    ...
    is still critical for the normal type checking of accessors, because without it there
    could be no subtyping. [Accessor] addresses this problem by exposing its
    representation as a function and offering a syntax extension, as part of ppx_accessor,
    to eta expand the definition for you. The upshot is that you can define accessors like
    this and not think about the value restriction anymore:
...
(** Many of the functions for creating accessors are documented with conditions that are
    ...
    are sufficiently restrictive that the type parameters are not quite freely independent
    of each other.

    The properties generally make intuitive sense, but it can still be useful sometimes to
    define an accessor that is not always well behaved. Even [Accessor] itself defines a
    few badly behaved accessors. If you define an accessor that isn't well behaved, it is
    recommended that you document the conditions under which it is not well behaved. *)

(** {2 Creation functions} *)

(** {3 Field accessors} *)

(** [field ~get ~set] creates a field accessor. A field accesses exactly one value within
    a composite data structure. For the field to be well behaved, [get] and [set] should
...
(** [field'] is the same as [field], just with a slightly different interface. [field] is
    usually more convenient to use, but [field'] can be useful to allow [get] and [set] to
    share the computation of finding the location to modify. *)
val field'
  :  ('at -> 'a * ('b -> 'bt))
  -> ('i -> 'a -> 'b, 'i -> 'at -> 'bt, [< field ]) General.t

(** A [Field.t] is sufficient to define a field accessor, but the resulting accessor might
    not be as polymorphic as it could have been if defined by hand or using
    [@@deriving accessor]. *)
val of_field
...
(** A [Field.t] is sufficient to define an indexed field accessor, where the index is the
    name of the field as a string. The resulting accessor might not be as polymorphic as
    it could have been if defined by hand or using [@@deriving accessor]. *)
val of_fieldi
  :  ([> `Set_and_create ], 'r, 'a) Base.Field.t_with_perm
...
(** [optional ~match_ ~set] creates an optional accessor. An optional accesses at most one
    value within a composite data structure. A well behaved optional should satisfy the
...
(** [filter_index predicate] accesses the entire value if its index satisfies [predicate],
    otherwise it accesses nothing. Compose it with a many accessor to access a subset of
    values. *)
val filter_index
  :  ('i Index.t -> bool)
...
(** [isomorphism ~get ~construct] creates an isomorphism accessor. An isomorphism accesses
    exactly one value which exactly represents the entire data structure. A well behaved
    isomorphism should satisfy the following properties:
...
(** [mapper map] creates a mapper accessor. A mapper can modify values inside a composite
    data structure, but cannot read anything out. A well behaved mapper should satisfy the
    following properties:
...
(** [nonempty traverse] creates a nonempty accessor. A nonempty accesses a nonzero number
    of values within a composite data structure.
...
(** [optional_getter get] creates an optional getter accessor. An optional getter reads at
    most one value from a composite data structure. There are no properties necessary for
    an optional getter to be well behaved. *)
val optional_getter
  :  ('at -> 'a option)
...
(** Turn a getter into a constructor. [getter_to_constructor (getter f)] is
    [constructor f]. *)
val getter_to_constructor
  :  (unit -> 'a -> _, unit -> 'at -> _, [> getter ]) General.t
  -> (_ -> _ -> 'at, _ -> _ -> 'a, [< constructor ]) General.t

(** Turn a constructor into a getter. [constructor_to_getter (constructor f)] is
    [getter f]. *)
