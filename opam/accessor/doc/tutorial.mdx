...
`Accessor` is suitable for use in codepaths where performance is not a concern. Use of
`Accessor` costs allocation at runtime, even when working with immutable data.
